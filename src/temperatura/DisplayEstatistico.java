...
public class DisplayEstatistico implements Observador {
	...
	public void atualizar(double temperatura, double humidade, double pressao) {
		...
		this.temperaturaMedia = Math
				.round(this.ultimos10Dados.stream().mapToDouble(DadosVO::getTemperatura).average().orElse(0.0) * 100.0)
				/ 100.0;

		this.humidadeMedia = Math.round(
				this.ultimos10Dados.stream().mapToDouble(DadosVO::getHumidade).average().orElse(0.0) * 100.0) / 100.0;

		this.temperaturaMax = Math.round(
				this.getUltimos10Dados().stream().map(DadosVO::getTemperatura).max(Double::compare).orElse(0.0) * 100.0)
				/ 100.0;

		this.temperaturaMin = Math.round(
				this.getUltimos10Dados().stream().map(DadosVO::getTemperatura).min(Double::compare).orElse(0.0) * 100.0)
				/ 100.0;
